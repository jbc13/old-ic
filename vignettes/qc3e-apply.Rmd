...
```{r}
# Definições iniciais
id_qc <- '3e' # identificador do teste de controle de qualidade aplicado
id_title <- paste0('QC_', id_qc) # identificador usado nos mapas e gráficos
prefix <- 'tmax-data-inmet-2008-2016-4yrs-south'
```
...
filter(var_data_qc, tmax_qc3e == 1) %>% arrange(site)
```
...
## var_nr_susp <- var_data_qc %>% filter(tmax_qc3e == 1) %>% nrow()
