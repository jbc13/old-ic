...
```{r}
# Definições iniciais
id_qc <- '3c' # identificador do teste de controle de qualidade aplicado
...
dew_file <- '/home/jonas/Documents/GITHUB/jbc-ic/data_saves/data_tdavg_filter.rds'

# Importação dos dados filtrados
dew_data <- readRDS(dew_file) %>%
  arrange(site) %>%
...
vardew_data <- data.table::data.table(var_data, dew_data)
vardew_data
...
var_data_qc <- qc_3c(
  data = vardew_data, 
  variable = "tavg",
  var_dew = "tdavg_filter") 
var_data_qc

filter(var_data_qc, tavg_qc3c == 1) %>% arrange(site)
```
...
## var_nr_susp <- var_data_qc %>% filter(tavg_qc3c == 1) %>% nrow()
...
summary_qc <-
  ...
  summarise(
    qc = first(qc),
    tot = sum(duration), # tot: total de dados suspeitos por site (em horas)e
    n_events = max(id),
    n_max = max(duration)) %>%
  ungroup() %>%
