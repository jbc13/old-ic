...
```{r}
# Definições iniciais
id_qc <- '3f' # identificador do teste de controle de qualidade aplicado
id_title <- paste0('QC_', id_qc) # identificador usado nos mapas e gráficos
var <- 'tmin' # Variável 1
var2 <- 'tmax' # Variável 2
prefix <- 'tmin-data-inmet-2008-2016-4yrs-south'
```
...
filter(var_data_qc, tmin_qc3f == 1) %>% arrange(site)
```
...
## var_nr_susp <- var_data_qc %>% filter(tmin_qc3f == 1) %>% nrow()
