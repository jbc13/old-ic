...
```{r}
# Definições iniciais
id_qc <- '1a' # identificador do teste de controle de qualidade aplicado
...
filter(var_data_qc, tavg_qc1a == 1) %>% arrange(site)
```
...
## var_nr_susp <- var_data_qc %>% filter(tavg_qc1a == 1) %>% nrow()
